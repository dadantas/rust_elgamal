...
#[unsafe(no_mangle)]
pub extern "C" fn encrypt_message(pub_key: *const u8, message: *const u8) -> *mut u8 {
    if pub_key.is_null() || message.is_null() {
        return std::ptr::null_mut();
    }
    let pub_key = unsafe { std::slice::from_raw_parts(pub_key, 64) };
    let message = unsafe { std::slice::from_raw_parts(message, 32) };
...
#[unsafe(no_mangle)]
pub extern "C" fn decrypt_message(priv_key: *const u8, ciphertext: *const u8) -> *mut u8 {
    let priv_key = unsafe { std::slice::from_raw_parts(priv_key, 32) };
    let ciphertext = unsafe { std::slice::from_raw_parts(ciphertext, 160) };
...
#[unsafe(no_mangle)]
pub extern "C" fn rerandomize_ciphertext(ciphertext: *const u8) -> *mut u8 {
    let ciphertext = unsafe { std::slice::from_raw_parts(ciphertext, 160) };

    let rerandomized_ciphertext = cipher::elgamal::rerandomize_bytes(ciphertext).unwrap();
